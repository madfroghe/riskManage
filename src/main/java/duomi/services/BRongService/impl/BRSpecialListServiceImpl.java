 ...
 */
@Service
public class BRSpecialListServiceImpl implements BRSpecialListService {
    @Autowired
    ...
    private ComResponse<JSONObject> getInfoFromLocal(BRSpecialListRequest request,
    ...
    private ComResponse<JSONObject> getInfoFromOutside(BRSpecialListRequest request)
            throws Exception {
        // 接受到调度请求后，登记外部服务查询状态表
        regitSrv.insertCspStus(request);

        // 发送外部服务
        JSONObject output = bRongHttpService.getSpecialList(request);
        ...
        mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_BR_SPECIAL_LIST);
