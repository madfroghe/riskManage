...
@Service
public class MultipleCreditAppServiceImpl implements MultipleCreditAppService {
	...
	public ComResponse<MultipleCreditAppResult> getMultipleCreditApp(MultipleCreditAppRequest request)
			throws Exception {
		ComResponse<MultipleCreditAppResult> output = null;
	...
	private ComResponse<MultipleCreditAppResult> getFourInfoFromOutside(MultipleCreditAppRequest request)
			throws Exception {
		// 接受到调度请求后，登记外部服务查询状态表
		regitSrv.insertCspStus(request);

		// 发送外部服务 BackCardThreeELementOutput
		BaseResponse<MultipleCreditAppResult> output = httpservice.checkMultipleCreditApp(request);
		...
		mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_ZB_MULTIPLECREDITAPPE);
		// 返回消息转换
		ComResponse<MultipleCreditAppResult> rsp = new ResponseSimpleHelper<MultipleCreditAppResult>()
				.ConvertRsp(request, output);

		return rsp;
	}

	private ComResponse<MultipleCreditAppResult> getFourInfoFromLocal(MultipleCreditAppRequest request,
			CspInterfaceStatPoWithBLOBs cspStatPo) {
		String retMsg = cspStatPo.getRetMessage();

		GsonUtils<MultipleCreditAppResult> gson = new GsonUtils<MultipleCreditAppResult>();
		BaseResponse<MultipleCreditAppResult> output = (BaseResponse<MultipleCreditAppResult>) gson.fromJson(retMsg,
				BaseResponse.class, MultipleCreditAppResult.class);
