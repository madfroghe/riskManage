 ...
 */
@Service
public class PersonJudicialListServiceImpl implements PersonJudicialListService {
    ...
    public ComResponse<JSONObject> personJudicialList(PersonJudicialListRequest request) throws Exception {
     ...
     * 从本地数据库获取
     * */
    private ComResponse<JSONObject> getInfoFromLocal(PersonJudicialListRequest request,
        ...
        BaseResponse<JSONObject> output = (BaseResponse<JSONObject>) gson.fromJson(retMsg, BaseResponse.class,
                JSONObject.class);
     ...
     * 从外部数据库获取
     * */
    private ComResponse<JSONObject> getInfoFromOutside(PersonJudicialListRequest request) throws Exception {
        // 接受到调度请求后，登记外部服务查询状态表
        regitSrv.insertCspStus(request);

        // 发送外部服务
        BaseResponse<JSONObject> output = httpservice.getPersonJudicialList(request);
        ...
        mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_ZB_PERSONJUDICIALLIST);
