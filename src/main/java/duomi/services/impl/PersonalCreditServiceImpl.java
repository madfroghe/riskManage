...
public class PersonalCreditServiceImpl implements PersonalCreditService {
    ...
    public ComResponse<JSONObject> personalCredit(PersonalCreditRequest request) throws Exception {
     ...
     * 从本地数据库获取
     * */
    private ComResponse<JSONObject> getInfoFromLocal(PersonalCreditRequest request,
     ...
     * 从外部数据库获取
     * */
    private ComResponse<JSONObject> getInfoFromOutside(PersonalCreditRequest request) throws Exception {
        // 接受到调度请求后，登记外部服务查询状态表
        regitSrv.insertCspStus(request);

        // 发送外部服务
        BaseResponse<JSONObject> output = httpservice.getPersonalCredit(request);
        ...
        mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_ZB_PERSONALCREDIT);
