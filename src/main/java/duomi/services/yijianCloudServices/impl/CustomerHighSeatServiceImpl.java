...
import duomi.services.yijianCloudServices.CustomerHighSeatService;
 ...
 */
@Service
public class CustomerHighSeatServiceImpl implements CustomerHighSeatService {
    ...
    public ComResponse<JSONObject> customerHighSeat(CustomerHighSeatRequest request) throws Exception {
     ...
     * 从本地数据库获取信息
     * */
    private ComResponse<JSONObject> getInfoFromLocal(CustomerHighSeatRequest request,
     ...
     * 从外部资信获取信息
     * */
    private ComResponse<JSONObject> getInfoFromOutside(CustomerHighSeatRequest request)
            throws Exception {
        // 接受到调度请求后，登记外部服务查询状态表
        regitSrv.insertCspStus(request);

        // 发送外部服务
        YiJianCloudResponse<JSONObject> output = yjCloudHttpService.customerHighSeat(request);
        ...
        mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_YJY_CUSTOMERHIGHSEAT);
