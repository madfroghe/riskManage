...
public class EducationServiceImpl implements EducationService {
    ...
    public ComResponse<JSONObject> education(EducationRequest request) throws Exception {
     ...
     * 从本地数据库获取信息
     * */
    private ComResponse<JSONObject> getInfoFromLocal(EducationRequest request,
     ...
     * 从外部资信获取信息
     * */
    private ComResponse<JSONObject> getInfoFromOutside(EducationRequest request)
            throws Exception {
        // 接受到调度请求后，登记外部服务查询状态表
        regitSrv.insertCspStus(request);

        // 发送外部服务
        YiJianCloudResponse<JSONObject> output = yjCloudHttpService.education(request);
        ...
        mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_YJY_EDUCATION);
