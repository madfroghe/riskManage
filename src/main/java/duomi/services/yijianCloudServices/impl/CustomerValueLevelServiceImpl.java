...
import duomi.services.yijianCloudServices.CustomerValueLevelService;
 ...
 */
@Service
public class CustomerValueLevelServiceImpl implements CustomerValueLevelService {
    @Autowired
     ...
     * 从本地数据库获取信息
     * */
    private ComResponse<JSONObject> getInfoFromLocal(CustomerValueLevelRequest request,
     ...
     * 从外部资信获取信息
     * */
    private ComResponse<JSONObject> getInfoFromOutside(CustomerValueLevelRequest request)
            throws Exception {
        // 接受到调度请求后，登记外部服务查询状态表
        regitSrv.insertCspStus(request);

        // 发送外部服务
        YiJianCloudResponse<JSONObject> output = yjCloudHttpService.customerValueLevel(request);
        ...
        mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_YJY_CUSTOMER_VALUELEVEL);
