 ...
 */
@Service
public class QueryEntByPersonServiceImpl implements QueryEntByPersonService {
     ...
     * 从本地数据库获取信息
     * */
    private ComResponse<JSONObject> getInfoFromLocal(QueryEntByPersonRequest request,
     ...
     * 从外部资信获取信息
     * */
    private ComResponse<JSONObject> getInfoFromOutside(QueryEntByPersonRequest request)
            throws Exception {
        // 接受到调度请求后，登记外部服务查询状态表
        regitSrv.insertCspStus(request);

        // 发送外部服务
        YiJianCloudResponse<JSONObject> output = yjCloudHttpService.queryEntByPerson(request);
        ...
        mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_YJY_QUERY_ENT_BY_PERSON);
