 ...
 */
@Service
public class FinanceBlackListServiceImpl implements FinanceBlackListService {
    @Autowired
    ...
    public ComResponse<JSONObject> financeBlackList(FinanceBlackListRequest request) throws Exception {
     ...
     * 从本地数据库获取信息
     * */
    private ComResponse<JSONObject> getInfoFromLocal(FinanceBlackListRequest request,
     ...
     * 从外部资信获取信息
     * */
    private ComResponse<JSONObject> getInfoFromOutside(FinanceBlackListRequest request)
            throws Exception {
        // 接受到调度请求后，登记外部服务查询状态表
        regitSrv.insertCspStus(request);

        // 发送外部服务
        YiJianCloudResponse<JSONObject> output = yjCloudHttpService.financeBlackList(request);
        ...
        mongodbBaseDao2.save(mgJson, MongoDbCollectionConstants.T_DM_CSP_YJY_FINANCE_BLACK_LIST);
