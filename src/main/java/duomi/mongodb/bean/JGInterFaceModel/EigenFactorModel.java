package duomi.mongodb.bean.JGInterFaceModel;

import com.alibaba.fastjson.JSONObject;
import com.mongodb.DBObject;
import duomi.com.httpIvk.param.JiGResponse;
import duomi.dbMap.bean.EigenfactorPo;
import duomi.mongodb.dao.annotation.QueryField;
import duomi.mongodb.dao.annotation.QueryType;
import org.springframework.data.annotation.Id;
import org.springframework.data.mongodb.core.mapping.Document;

/**
 * Created by DEllComputer on 2018/1/31.
 *
 * 极光反欺诈
 */
@Document(collection = "t_dm_csp_eigenFactor")
public class EigenFactorModel {
    @Id
    @QueryField
    private String id;

    @QueryField(type = QueryType.EQUALS, attribute = "appno")
    private String appno;

    @QueryField(type = QueryType.LIKE, attribute = "name")
    private String name;

    @QueryField(type = QueryType.EQUALS, attribute = "mobiel")
    private String mobiel;

    @QueryField
    private String idCard;

    @QueryField(type = QueryType.EQUALS,attribute = "interSerno")
    private String interSerno;

    @QueryField(type = QueryType.IN, attribute = "data")
    private JiGResponse<EigenfactorPo> data;

    @QueryField(type = QueryType.IN, attribute = "analyJSON")
    private net.sf.json.JSONObject analyJSON;

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getAppno() {
        return appno;
    }

    public String getInterSerno() {
        return interSerno;
    }

    public void setInterSerno(String interSerno) {
        this.interSerno = interSerno;
    }

    public void setAppno(String appno) {
        this.appno = appno;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getMobiel() {
        return mobiel;
    }

    public void setMobiel(String mobiel) {
        this.mobiel = mobiel;
    }

    public String getIdCard() {
        return idCard;
    }

    public void setIdCard(String idCard) {
        this.idCard = idCard;
    }

    public JiGResponse<EigenfactorPo> getData() {
        return data;
    }

    public void setData(JiGResponse<EigenfactorPo> data) {
        this.data = data;
    }

    public net.sf.json.JSONObject getAnalyJSON() {
        return analyJSON;
    }

    public void setAnalyJSON(net.sf.json.JSONObject analyJSON) {
        this.analyJSON = analyJSON;
    }
}
