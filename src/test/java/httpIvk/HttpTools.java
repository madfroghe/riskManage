...
public class HttpTools {
	...
	public static CloseableHttpClient createInstance(int connectTimeout, int socketTimeout) {
		...
		RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(connectTimeout)
				.setSocketTimeout(socketTimeout).setConnectionRequestTimeout(connectTimeout).setRedirectsEnabled(true)
				.setStaleConnectionCheckEnabled(true).setCookieSpec("easy").build();
	...
	public static String postJson(String url, String params, int connectTimeout, int readTimeout) throws Exception {
		CloseableHttpClient httpclient = null;
		HttpPost httppost = new HttpPost(url);
		try {
			httpclient = HttpTools.createInstance(connectTimeout, readTimeout);

			StringEntity bodyEntity = new StringEntity(params, "UTF-8");
